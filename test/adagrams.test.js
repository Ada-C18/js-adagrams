...
describe('Adagrams', () => {
  const newAdagrams = new Adagrams();
  ...
  describe('usesAvailableLetters', () => {
    ...
    it('returns true if the submitted letters are valid against the drawn letters', () => {
      ...
      const isValid = newAdagrams.usesAvailableLetters(word, drawn);
  ...
  describe('scoreWord', () => {
    ...
    const expectScores = (wordScores) => {
      Object.entries(wordScores).forEach(([word, score]) => {
        expect(newAdagrams.scoreWord(word)).toBe(score);
      });
    };
  ...
  describe('highestScoreFrom', () => {
    ...
    it('returns a hash that contains the word and score of best word in an array', () => {
      ...
      const correct = { word: 'XXXX', score: newAdagrams.scoreWord('XXXX') };

      expect(newAdagrams.highestScoreFrom(words)).toEqual(correct);
    });
    ...
    describe('in case of tied score', () => {
      const expectTie = (words) => {
        const scores = words.map((word) => newAdagrams.scoreWord(word));
      ...
      it('selects the word with 10 letters', () => {
        ...
        const correct = {
          word: 'AAAAAAAAAA',
          score: newAdagrams.scoreWord('AAAAAAAAAA')
        };
        expectTie(words);

        expect(newAdagrams.highestScoreFrom(words)).toEqual(correct);
        expect(newAdagrams.highestScoreFrom(words.reverse())).toEqual(correct);
      });

      it('selects the word with fewer letters when neither are 10 letters', () => {
        const words = ['MMMM', 'WWW'];
        const correct = { word: 'WWW', score: newAdagrams.scoreWord('WWW') };
        expectTie(words);
      ...
      it('selects the first word when both have same length', () => {
        ...
        const second = {
          word: 'EEEEEEEEEE',
          score: newAdagrams.scoreWord('EEEEEEEEEE')
        };
        expectTie(words);

        expect(newAdagrams.highestScoreFrom(words)).toEqual(first);
        expect(newAdagrams.highestScoreFrom(words.reverse())).toEqual(second);
      });
    });
  });
});
